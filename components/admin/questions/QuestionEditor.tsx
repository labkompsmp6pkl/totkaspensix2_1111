...
// PERBAIKAN: Menghapus React.memo. Penggunaan React.memo dengan props function yang tidak
// di-useCallback (seperti setForm, onSave) akan membuat komponen nge-bug dan membeku.
const QuestionEditor: React.FC<QuestionEditorProps> = ({
  form, setForm, groups, subjects, autoCalculatedPoints, isSaving, onSave, onClose, handleModeChange
}) => {
  return (
    // Z-Index yang ekstrim tinggi dan pointer-events-auto memastikan tidak ada elemen background
    // (seperti Sidebar atau Navbar) yang mencuri "klik" dari modal ini.
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl z-[9999] flex items-center justify-center p-2 sm:p-6 overflow-hidden pointer-events-auto">
       <div className="bg-white w-full max-w-6xl rounded-[3rem] sm:rounded-[4rem] p-6 sm:p-12 shadow-2xl overflow-y-auto max-h-[95vh] space-y-8 animate-in zoom-in-95 border-8 border-white relative">
          ...
          <button 
            onClick={onClose} 
            className="absolute top-8 right-8 w-12 h-12 flex items-center justify-center bg-slate-50 rounded-full text-slate-300 hover:text-red-500 hover:bg-red-50 font-black text-2xl transition-all cursor-pointer"
          >
            ✕
          </button>
          ...
          <div className="flex flex-col gap-6 border-b pb-8">
             ...
             <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="space-y-1">
                   ...
                   <select 
                      className="w-full p-4 bg-slate-50 border rounded-2xl font-black text-[10px] text-blue-900 outline-none cursor-pointer" 
                      value={form.subject || ''} 
                ...
                <div className="space-y-1">
                   ...
                   <div className="relative">
                      {form.scoring_mode === 'all_or_nothing' ? (
                        <input 
                          type="number" 
                          className="w-full p-4 bg-blue-50 border-2 border-blue-100 rounded-2xl font-black text-blue-900 text-center" 
                          value={form.points || 0} 
                ...
                <div className="space-y-1">
                   ...
                   <input 
                      type="number" 
                      className="w-full p-4 bg-slate-50 border rounded-2xl font-black text-center" 
                  ...
                  {groups.map(g => {
                    ...
                    return (
                      <button 
                        ...
                        className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase border-2 transition-all active:scale-95 cursor-pointer ${isSelected ? 'bg-blue-900 text-white border-blue-950 shadow-md' : 'bg-white text-slate-300 border-slate-100 hover:border-blue-300 hover:text-blue-500'}`}
                      >
                        {g.group_name}
                      </button>
                    );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
             <textarea 
                className="w-full p-6 bg-slate-50 border-2 border-slate-50 rounded-[2.5rem] min-h-[160px] outline-none text-[12px] font-semibold focus:bg-white focus:border-blue-900 transition-all shadow-inner" 
                value={form.text || ''} 
          ...
          <div className="bg-slate-50/50 p-6 sm:p-10 rounded-[4rem] border-2 border-slate-100 space-y-8 shadow-inner">
             <div className="flex flex-col sm:flex-row justify-between items-center border-b border-slate-200 pb-4 gap-4">
               ...
               {form.type === 'table' && (
                 <div className="flex gap-2 p-2 bg-white rounded-2xl border shadow-sm">
                   ...
                   <input className="w-24 p-2 bg-slate-50 border rounded-lg text-[9px] font-black text-center uppercase focus:border-blue-900 outline-none" placeholder="KOLOM 1" value={form.tableOptions?.[0] || 'BENAR'} onChange={e => {
                     const next = [...(form.tableOptions || ['BENAR', 'SALAH'])];
                     next[0] = e.target.value.toUpperCase();
                     setForm({...form, tableOptions: next});
                   }} />
                   <input className="w-24 p-2 bg-slate-50 border rounded-lg text-[9px] font-black text-center uppercase focus:border-blue-900 outline-none" placeholder="KOLOM 2" value={form.tableOptions?.[1] || 'SALAH'} onChange={e => {
                     const next = [...(form.tableOptions || ['BENAR', 'SALAH'])];
             ...
             {(form.type === 'single' || form.type === 'multiple') && (
               <div className="grid grid-cols-1 gap-6">
                     ...
                     <div key={opt.id} className={`bg-white p-6 rounded-[2.5rem] border-2 transition-all shadow-sm flex flex-col gap-6 ${isCorrect ? 'border-blue-200 bg-blue-50/20' : 'border-slate-100'}`}>
                        <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
                           <div className="flex items-center gap-4 shrink-0">
                              ...
                              <span className={`w-12 h-12 flex items-center justify-center rounded-xl font-black text-sm cursor-pointer ${isCorrect ? 'bg-blue-900 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}
                                    onClick={() => {
                                      // Tambahkan fungsi klik pada label huruf agar lebih mudah diklik
                                      if (form.type === 'single') setForm({...form, correctOptionId: opt.id});
                                      else {
                                        const cur = ensureArray(form.correctOptionIds);
                                        const next = cur.includes(opt.id) ? cur.filter(id => id !== opt.id) : [...cur, opt.id];
                           ...
                           <button onClick={() => { 
                             ...
                             setForm({...form, options: next}); 
                           }} className="p-3 bg-red-50 text-red-300 rounded-full hover:bg-red-500 hover:text-white transition-all cursor-pointer">✕</button>
                        </div>
                     </div>
                   );
                 })}
                 ...
                 }} className="w-full py-5 border-4 border-dashed border-slate-200 text-slate-300 font-black text-[11px] uppercase rounded-[2.5rem] hover:border-blue-200 hover:text-blue-500 hover:bg-blue-50/50 transition-all cursor-pointer">+ TAMBAH OPSI JAWABAN</button>
               </div>
             )}
             ...
             {form.type === 'table' && (
               <div className="space-y-6">
                             ...
                             <div className="flex gap-1">
                               {ensureArray(form.tableOptions).map(to => (
                                 <button key={to} onClick={() => {
                                   ...
                                   setForm({...form, statements: next});
                                 }} className={`flex-1 px-4 py-3 rounded-xl text-[9px] font-black uppercase border transition-all cursor-pointer ${st.correctAnswer === to ? 'bg-blue-900 text-white shadow-md border-blue-950' : 'bg-slate-50 text-slate-300 hover:bg-slate-100'}`}>{to}</button>
                               ))}
                             </div>
                          </div>
                          {form.scoring_mode === 'partial' && (
                  ...
                  <button onClick={() => setForm({...form, statements: [...ensureArray(form.statements), { id: Date.now().toString(), text: '', points: 5, correctAnswer: ensureArray(form.tableOptions)?.[0] || 'BENAR' }]})} className="w-full py-5 border-4 border-dashed border-slate-200 text-slate-300 font-black text-[11px] uppercase rounded-[2.5rem] hover:border-blue-200 hover:text-blue-500 hover:bg-blue-50/50 transition-all cursor-pointer">+ TAMBAH BARIS PERNYATAAN</button>
               </div>
             )}
          </div>

          <div className="flex gap-4 pt-10 border-t">
             <button onClick={() => { console.log("Batal"); onClose(); }} className="flex-1 py-6 bg-slate-100 rounded-[2rem] font-black uppercase text-[11px] text-slate-400 hover:bg-slate-200 transition-all cursor-pointer">BATAL</button>
             <button onClick={() => { console.log("Simpan"); onSave(); }} disabled={isSaving} className="flex-[2] py-6 bg-blue-900 text-white rounded-[2rem] font-black uppercase text-[12px] shadow-2xl border-b-8 border-blue-950 active:scale-95 transition-all tracking-[0.2em] cursor-pointer disabled:opacity-50">
