...
import { Question, QuestionType, ScoringMode, QuestionGroup, User } from '../../../types';
...
const QuestionForm: React.FC<QuestionFormProps> = ({
  form, setForm, groups, onClose, onSuccess, editingId, API_BASE_URL, currentUser
}) => {
  const [isSaving, setIsSaving] = useState(false);
  const subjects = ['MATEMATIKA', 'BAHASA INDONESIA', 'BAHASA INGGRIS', 'IPA', 'IPS', 'PPKN', 'PAI'];

  const autoCalculatedPoints = useMemo(() => {
    if (form.scoring_mode === 'all_or_nothing') return form.points || 0;
    const options = ensureArray(form.options);
    return options.reduce((sum, opt) => sum + (opt.points || 0), 0);
  }, [form.options, form.scoring_mode, form.points]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await QuestionController.save(dataToSave);
  ...
  const handleModeChange = (mode: ScoringMode) => {
    setForm({ ...form, scoring_mode: mode });
  };
  return (
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl z-[99999] flex items-center justify-center p-2 sm:p-6 overflow-hidden pointer-events-auto">
       <div className="bg-white w-full max-w-6xl rounded-[3rem] sm:rounded-[4rem] p-6 sm:p-12 shadow-2xl overflow-y-auto max-h-[95vh] space-y-8 animate-in zoom-in-95 border-8 border-white relative z-[100000]">
          ...
          <div className="flex gap-4 pt-10 border-t border-slate-100 relative z-[200]">
             ...
             <button onClick={() => { console.log("Simpan"); handleSave(); }} disabled={isSaving} className="flex-[2] py-6 bg-blue-900 text-white rounded-[2rem] font-black uppercase text-[12px] shadow-2xl border-b-8 border-blue-950 active:scale-95 transition-all tracking-[0.2em] cursor-pointer hover:bg-blue-800 disabled:opacity-50 hover:-translate-y-0.5 pointer-events-auto z-[200]">
